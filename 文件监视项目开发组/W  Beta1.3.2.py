...
class SystemSettings(wx.Frame):
    ...
    def SizeRank_ForBackUp(self, inputlist, methond = 0):
        ...
        temp1 = sorted(d.items(), key = lambda x:x[1], reverse = methond)
    ...
    def AutoClean(self):
        if len(self.filenames) > 50:
            ...
            self.ErrorRecord.append('内存占用自动清理完毕')
            ...
            try:
                ...
                try:
                    ...
                    if len(self.ErrorRecord) != 0:
                        ...
                        self.s7.SetValue('欢迎使用文件监测备份系统，首次使用请转到系统设置界面进行初次设置')
                    if self.Full == -1:
                        pass
                    else:
                        ...
                        if Percent > 90:
                            if self.WarnedState == False:
                                ...
                                InFo1 = wx.MessageDialog(None,"备份空间使用已经大于90%", "警告")
                                if InFo1.ShowModal() == wx.ID_YES:
                                    InFo1.Destroy()
    ...
    def StartUpSystemSettings(self, event):
        ...
        self.checkbox5 = wx.CheckBox(self.panel, -1, '开启内存定时清理', size = (120, 20), pos = (X_Distance + 30, Y_Distance + 495))
