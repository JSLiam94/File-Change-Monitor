...
class SystemSettings(wx.Frame):
    def __init__(self):
        ...
        self.FormatDic = {0:'all', 1:'txt', 2:'wma.wmv.rm.rmvb.aac.mid.wav.vqf.avi.cda',
                          3:'avi.rmvb.rm.asf.divx.mpg.mpeg.mpe.wmv.mp4.mkv.vob',
                          4:'bmp.jpg.jpeg.png.gif', 5:'ppt.pptx', 6:'xls.xlsx',
                          7:'doc.docx', 8:'mdb.db.mdf.dbf', 9:'reg', 10:'ico', 11:'ink'}
        ...
        self.BackUppedSize = 0
        self.BackUppedNumber = 0
        self.time3 = -1
        self.Full = -1
        self.WarnedState = False
        self.ErrorRecord = []
        self.TargetSize = -1
        self.NoneType = False
        self.MonitorSettingsState = False
        self.LastSelection_one = 0
        self.LastSelection_Two = 0
    def Initialization(self, originalpath, copypath, SelectCode = 0, Monitormode = 0):
        ...
        self.Full = int(self.EntryText6.GetValue()) * 2 ** 20
        if self.listbox2.GetSelection() == 0 and self.checkbox2.GetValue() == True:
            self.Sizeselect = True
        self.listbox3Selection = self.listbox3.GetSelection()
        self.Entrytext2Value = self.EntryText2.GetValue()
        self.Entrytext3Value = self.EntryText3.GetValue()
        self.RankMode = -1
        self.AutoCleanState = False
        if self.listbox5.GetSelection() == -1:
            self.RankMode = -1
        if self.listbox5.GetSelection() == 0:
            self.RankMode = 0
        if self.listbox5.GetSelection() == 1:
            self.RankMode = 1
        if self.checkbox5.GetValue() == False:
            self.AutoCleanState = False
        else:
            self.AutoCleanState = True
        self.OverFlowState = self.listbox6.GetSelection()
    def AutoMemoryUsageControl(self):
        if self.TargetSize == -1:
            return None
        self.TargetSize = int(self.TargetSize) * 1024 ** 2
        while True:
            time.sleep(1)
            Current = self.MemoryCheck()
            if int(Current) >= self.TargetSize:
                self.AutoClean(1)
    def SizeRank_ForBackUp(self, inputlist, methond = 0):
        if methond == -1:
            return None
        finall_list = []
        ...
        for element in inputlist:
            temp_dic[element] = os.stat(element).st_size
        temp1 = sorted(temp_dic.items(), key = lambda x:x[1], reverse = methond)
        for element in temp1:
            finall_list.append(element[0])
        return finall_list
    def SizeSelection(self, inputsize):
        SizeDic = {0 : 0, 1 : 1024 ** 1, 2 : 1024 ** 2, 3 : 1024 ** 3, 4 : 1024 ** 4}
        Convey = SizeDic[self.listbox3Selection]
        Min = int(self.Entrytext2Value) * Convey
        Max = int(self.Entrytext3Value) * Convey
        if Min <= inputsize <= Max:
            return True
        else:
            self.ErrorRecord.append('检测到的文件变动不符合筛选要求的文件变动，未备份')
            return False
    def AutoClean(self, code = 0):
        Size = 5
        if code == 1:
            Size = 5
            String = '内存占用自动清理完毕(内存使用触及设定上限)' + time.strftime('%Y-%m-%d %H-%M-%S', time.localtime())
            if self.TargetSize < 64 * 1024 ** 2:
                String += '若内存使用大小限制过于苛刻，很可能导致限制的值无法达到限制要求，会比限制略高并稳定在该数值'
        else:
            Size = 80
            String = '内存占用自动清理完毕(定量计划)' + time.strftime('%Y-%m-%d %H-%M-%S', time.localtime())
        if len(self.filenames) > Size:
            ...
            if len(self.ErrorRecord) > Size:
                self.ErrorRecord = self.ErrorRecord[-3:]
            if len(self.LastSettingState) > Size:
                self.LastSettingState = self.LastSettingState[-3:]
            if len(self.copyrecord) > Size:
                self.copyrecord = self.copyrecord[-3:]
            self.ErrorRecord.append(String)
            print(1)
    ...
    def _copyfilesinlistwithtime(self, inputlist, mode):
        ...
        for element in inputlist:
            ...
            if self.Sizeselect == True:
                fileinfo = os.stat(element)
                if self.SizeSelection(fileinfo.st_size) == True:
                    pass
                else:
                    continue
            ...
            self.ErrorRecord.append(target +'    已备份')
            self.BackUppedSize += fileinfo.st_size
            self.BackUppedNumber += 1
    def _formatbyte(self, inputsize):
        ...
        if len(byte.split('.')[0]) == 2:
            return (byte[: 2] if byte.endswith('.00') else byte) + '字节'
        else:
    ...
    def _shiftfilepaths(self, inputlist, targetpath):
        ...
        return templist
    def GetDirSize(self, incomepath):
        Total = 0
        Select = self.FormatDic[self.SelectCode]
        ...
        for element_one in os.walk(incomepath):
            if len(element_one[1]) == 0 and len(element_one[2]) != 0:
                for element_two in element_one[2]:
                    ...
                    fileinfo = os.stat(abspath)
                    Total += fileinfo.st_size
        return Total
    def DelFilesByRules(self, Rules):
        print(Rules)
        DelNumbers = 10
        Inputlist = self._listdir(self.copypath)
        if len(Inputlist) > DelNumbers:
            pass
        else:
            DelNumbers = len(Inputlist)
        RankedList = self.SizeRank_ForBackUp(Inputlist, Rules)
        for Index in range(DelNumbers):
            os.remove(RankedList[Index])
    def _listdir(self, incomepath):
        ...
        if Select == 'all':
            ...
            for element_one in os.walk(incomepath):
                if len(element_one[1]) == 0 and len(element_one[2]) != 0:
                    for element_two in element_one[2]:
                        Suffix = element_two.split('.')[1]
                        if Suffix in Selections:
                            abspath = os.path.join(element_one[0], element_two)
                            templist.append(abspath)
        return templist
    ...
    def _AnalysisData(self):
        ...
        try:
            ...
            if self.mode == 0 or self.mode == 1:
                self.SizeRank_ForBackUp(newfiles, self.RankMode)
    ...
    def StartUpStatePage(self):
        ...
        self.text229 = wx.StaticText(self.panel, -1, self.t235, pos = (X_Distance + 230, Y_Distance + 75))
        self.text230 = wx.StaticText(self.panel, -1, self.t236, pos = (X_Distance + 500, Y_Distance + 225))
        self.s1 = wx.TextCtrl(self.panel, -1, value ='', pos = (45 + 645, 15 + 200),size = (85, 20), style=wx.TE_READONLY)
        self.s2 = wx.TextCtrl(self.panel, -1, value ='', pos = (45 + 625, 15 + 175),size = (105, 20), style=wx.TE_READONLY)
        self.s3 = wx.TextCtrl(self.panel, -1, value ='', pos = (45 + 625, 15 + 125),size = (105, 20), style=wx.TE_READONLY)
        self.s4 = wx.TextCtrl(self.panel, -1, value ='', pos = (45 + 625, 15 + 100),size = (105, 20), style=wx.TE_READONLY)
        self.s5 = wx.TextCtrl(self.panel, -1, value ='', pos = (45 + 625, 15 + 75),size = (105, 20), style=wx.TE_READONLY)
        self.s6 = wx.TextCtrl(self.panel, -1, value ='', pos = (45 + 625, 15 + 150),size = (105, 20), style=wx.TE_READONLY)
        self.s7 = wx.TextCtrl(self.panel, -1, value ='', pos = (45 + 75, 15 + 110),size = (390, 220), style=wx.TE_READONLY|wx.TE_MULTILINE)
        self.s8 = wx.TextCtrl(self.panel, -1, value ='', pos = (45 + 625, 15 + 225),size = (105, 20), style=wx.TE_READONLY)
        ...
        self.text229.SetFont(self.font222)
        self.text230.SetFont(self.font223)
        self.s7.SetFont(self.font223)
        ...
        self.button11.Bind(wx.EVT_BUTTON, self.OpenBackUpDir)
        self.thread1 = threading.Thread(target = self.KeepUpDatingData)
        self.thread1.start()
    def OpenBackUpDir(self, event):
        if self.InitializationState == False:
            InFo1 = wx.MessageDialog(None,"未指定备份路径，请转到系统设置界面,设置完成后点击保存", "错误")
            if InFo1.ShowModal() == wx.ID_YES:
                InFo1.Destroy()
        else:
            os.startfile(self.copypath)
    def KeepUpDatingData(self):
        while True:
            try:
                ...
                try:
                    time.sleep(1)
                    self.s1.SetValue(self.t229)
                    self.s1.SetFont(self.font223)
                    self.s2.SetValue(self.t230)
                    self.s2.SetFont(self.font223)
                    final = self._formatbyte(self.MemoryCheck())
                    self.s8.SetValue(final)
                    self.s8.SetFont(self.font223)
                    self.t231 = self._formatbyte(self.BackUppedSize)
                    self.s3.SetValue(self.t231)
                    self.s3.SetFont(self.font223)
                    self.s4.SetValue(str(self.BackUppedNumber))
                    self.s4.SetFont(self.font223)
                    if self.InitializationState == True:
                        self.s6.SetValue(str(self.TypeNumbers()))
                        self.s6.SetFont(self.font223)
                    if self.time3 != -1:
                        t4 = time.time()
                        self.s5.SetValue(self.ProcessTime(int(t4 - self.time3)))
                    else:
                        self.s5.SetValue('0')
                    self.s5.SetFont(self.font223)
                    if len(self.ErrorRecord) != 0:
                        self.s7.SetValue(str(self.ErrorRecord[-1]))
                        self.s7.SetFont(self.font222)
                    else:
                        self.s7.SetValue('欢迎使用文件监测备份系统，首次使用请转到系统设置界面进行初次设置，因开发进度的原因，部分功能暂不可用，请期待更新')
                    if self.Full == -1:
                        pass
                    else:
                        CurSize = self.GetDirSize(self.copypath)
                        Percent = int(100 * (CurSize / self.Full))
                        if Percent < 100:
                            self.gauge.SetValue(Percent)
                        if Percent > 90:
                            if self.WarnedState == False:
                                self.WarnedState = True
                                if self.OverFlowState == 0:
                                    String = "备份空间使用已经大于90%(仅提示)"
                                    self.ErrorRecord.append(String)
                                if self.OverFlowState == 1:
                                    String = "备份空间使用已经大于90%(执行删除最小文件操作)"
                                    self.ErrorRecord.append(String)
                                    self.DelFilesByRules(0)
                                if self.OverFlowState == 2:
                                    String = "备份空间使用已经大于90%(执行删除最大文件操作)"
                                    self.ErrorRecord.append(String)
                                    self.DelFilesByRules(1)
                        if Percent < 90 and self.WarnedState == True:
                            self.WarnedState = False
                except RuntimeError as Error:
                    self.ErrorRecord.append(Error)
                    pass
    def TypeNumbers(self):
        temp = []
        templist = []
        try:
            for element in self.filenames[-1]:
                templist.append(element.split('.')[1])
            for element in templist:
                if temp.count(element) < 1:
                    temp.append(element)
        except IndexError as Error:
            if self.NoneType == False:
                self.NoneType = True
                self.ErrorRecord.append(str(Error) + '统计的文件存在无后缀文件，记录文件类型失败，可能会导致记录数据不准确')
            temp = []
            templist = []
            for element in self.filenames[-1]:
    ...
    def ProcessTime(self, Data):
        ...
        if Hours == 0:
            ...
            Temp = Temp + '0' + str(Hours) + ':' if Hours < 10 else Temp + str(Hours) + ':'
        if Mins == 0:
            Temp += '00:'
        else:
            Temp = Temp + '0' + str(Mins) + ':' if Mins < 10 else Temp + str(Mins) + ':'
        Temp = Temp + '0' + str(Secs) if Secs < 10 else Temp + str(Secs)
        return Temp
    ...
    def PressStart(self, event):
        if self.InitializationState == False:
            ...
            self.button001.Bind(wx.EVT_BUTTON,self.PressStop)
            self.EntryText0001 = wx.TextCtrl(self.panel, -1, size=(100, -1), pos = (61000, 9500))
            self.thread2 = threading.Thread(target = self.ActiveScan)
            self.thread2.start()
            if self.checkbox6.GetValue() == True:
                self.TargetSize = self.EntryText9.GetValue()
                self.thread3 = threading.Thread(target = self.AutoMemoryUsageControl)
                self.thread3.start()
    ...
    def Last(self, event):
        if self.StateCode == 0:
            pass
        if self.StateCode == 1:
            self.RecordLastMonitorSettings()
    ...
    def DestroyMonitorSettings(self, event):
        self.LastSelection_one = self.listbox01.GetSelection()
        self.LastSelection_Two = self.listbox03.GetSelection()
        self.font01.Destroy()
        self.font02.Destroy()
        self.font03.Destroy()
        self.font04.Destroy()
    ...
    def StartUpMonitorSettings(self, event):
        ...
        self.text06 = wx.StaticText(self.panel, -1, self.t06, size = (25,25), pos = (X_Distance + 535, Y_Distance + 68))
        ...
        self.listbox03 = wx.Choice(self.panel, -1, size = (80, 50), pos = (X_Distance + 635, Y_Distance + 65), choices=self.List03)
        ...
        self.DataList = self.ScreenTheList(self.DataList, self.LastSelection_one, self.LastSelection_Two)
        self.length = 0
        self.cur = 0
        self.total = 0
    ...
    def ScreenTheList(self, inputlist, LastSelection1, LastSelection2):
        templist = []
        templist1 = []
        target = self.FormatDic[LastSelection1]
        if target == 'all':
            templist = inputlist
            pass
        else:
            targetlist = target.split('.')
            if inputlist == None or inputlist == []:
                return None
            for element in inputlist:
                try:
                    if element.split('.')[1].split(':')[0] in targetlist:
                        templist.append(element)
                except IndexError as Error:
                    self.ErrorRecord.append(Error + '区分的文件没有后缀名(第一筛选)')
        if LastSelection2 == 0:
            return templist
        else:
            for element in templist:
                try:
                    if self.LastSelection_Two == 1:
                        if element.split('.')[0].split(']')[1] == '2':
                            templist1.append(element)
                    if self.LastSelection_Two == 2:
                        if element.split('.')[0].split(']')[1] == '4':
                            templist1.append(element)
                    if self.LastSelection_Two == 3:
                        if element.split('.')[0].split(']')[1] == '1':
                            templist1.append(element)
                except IndexError as Error:
                    self.ErrorRecord.append(Error + '区分的文件没有后缀名(第二筛选)')
        return templist1
    def RecordLastMonitorSettings(self):
        self.MonitorSettingsState = True
        self.Element_one = self.listbox01.GetSelection()
        self.Element_Two = self.listbox03.GetSelection()
    def LoadMonitorSettings(self):
        if self.MonitorSettingsState == False:
            pass
        else:
            self.listbox01.SetSelection(self.Element_one)
            self.listbox03.SetSelection(self.Element_Two)
    ...
    def StartUpSystemSettings(self, event):
        ...
        self.checkbox5 = wx.CheckBox(self.panel, -1, '内存自动定量清理', size = (120, 20), pos = (X_Distance + 30, Y_Distance + 495))
        self.checkbox6 = wx.CheckBox(self.panel, -1, '限制内存使用', size = (120, 20), pos = (X_Distance + 180, Y_Distance + 495))
        ...
        self.List5 = ['小文件优先', '大文件优先']
        self.List6 = ['仅提示', '删除最小的文件', '删除最大的文件']
        ...
        self.EntryText8.Enable(False)
        self.EntryText9.Enable(False)
        ...
        self.thread3 = threading.Thread(target = self.StateDetect)
        self.thread3.start()
    ...
    def CheckforProperty(self):
        ...
        if self.listbox4.GetSelection() == -1:
            return '请选择备份动作触发条件'
        try:
            int(self.EntryText6.GetValue())
        except ValueError as Error:
            self.ErrorRecord.append(Error)
            return '请设置正确的硬盘备份区大小限制数值'
        else:
            if int(self.EntryText6.GetValue()) < 0:
                return '请设置正确的硬盘备份区大小限制数值'
            if 0 < int(self.EntryText6.GetValue()) < 128:
                return '设置的备份区大小过小，请至少设置64MB及以上的大小'
        if self.checkbox6.GetValue() == True:
            try:
                int(self.EntryText9.GetValue())
            except ValueError as Error:
                self.ErrorRecord.append(Error)
                return '请设置正确的内存大小限制数值'
            else:
                if int(self.EntryText9.GetValue()) < 0:
                    return '请设置正确的内存大小限制数值'
                if 0 < int(self.EntryText9.GetValue()) < 32:
                    return '设置的可用内存大小过小，请至少设置32MB及以上的大小'
        if self.checkbox6.GetValue() == True and self.checkbox5.GetValue() == True:
            return '内存自动定量清理不能与限制内存使用同时开启，请选择其中一项'
        if self.listbox6.GetSelection() == -1:
            return '请选择备份区将满的动作'
        return True
    def Saved(self, event):
        if self.CheckforProperty() == True:
            ...
            InFo1 = wx.MessageDialog(None,'''保存成功，正在初始化(如有卡顿为正常现象，监测文件夹体积越大，初始化耗时越久)''', "消息提示")
            if InFo1.ShowModal() == wx.ID_YES:
                InFo1.Destroy()
            if self.checkbox1.GetValue() == True:
                self.Initialization(self.EntryText5.GetValue(), self.EntryText7.GetValue(),
                                    self.listbox1.GetSelection(), self.listbox4.GetSelection())
            else:
                self.Initialization(self.EntryText5.GetValue(), self.EntryText7.GetValue(),
        ...
        while True:
            ...
            try:
                ...
                if self.listbox2.GetSelection() == 0 and self.checkbox2.GetValue() == True:
                    ...
                    self.EntryText4.Enable(False)
                if self.listbox2.GetSelection() == 1 and self.checkbox2.GetValue() == True:
                    self.EntryText2.Enable(False)
                    self.EntryText3.Enable(False)
                    self.listbox3.Enable(False)
